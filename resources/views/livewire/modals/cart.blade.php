        ...
        @if (count($items) > 0)
            <footer class="space-y-6">
                ...
                <div class="flex flex-col">
                    <flux:button as:link href="{{ route('client.cart') }}" wire:navigate variant="primary"
                        class="!rounded-full w-full">Finalizar
                        compra
                    </flux:button>
                    <flux:button wire:click="clearCart" class="ml-auto mt-4" icon="backspace" size="sm" variant="subtle">Vaciar carrito
                    </flux:button>
                </div>
            </footer>
        @endif
